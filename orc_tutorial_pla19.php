...
$pxcAmis = array(
	"us-east-1" => "ami-03e685edc1b359b66",
	"us-east-2" => "ami-0ab93346f25031c4e",
	"us-west-1" => "ami-0f9a2f3ca6d61c9fe",
	"us-west-2" => "ami-071a755c2f6f55648");
...
function addNewCluster()
{
	...
	try
	{
		...
		{
			tagEntity($instanceId, 'Name', sprintf("PLA19-ORC-T%d-%s", $teamId, $types[$i]));
		}
	}
	catch(Exception $e)
	{
...
function saveInstanceInfoToDynamo($tid, $info)
{
	...
	$params = ['TableName' => 'pla19orctutorial', 'Item' => $row];
...
function saveAnsibleTeamConfig($teamId, $instanceInfo)
{
	...
	$file = sprintf("tutorial-orc-T%d.hosts", $teamId);
	file_put_contents($file, $a) or print("-- Unable to save ansible inventory\n");
	printf("\n ** ansible-playbook -i %s ansible_playbooks/orchestrator.yml\n", $file);
}
